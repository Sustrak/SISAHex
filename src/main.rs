...
fn create_hex(pos: &Instruction, s: &mut String, h: &HashMap<&str, i8>) {
    //opcode
    ...
    for l in contents.lines() {
        ...
        for item in dict.uses.get(ins_name).unwrap() {
            if item == "n" && (ins_name == "st" || ins_name == "ld") {
                ins_b.insert(item, c[item.as_str()].to_string().parse::<i8>().unwrap()/2);
            }
            else {
                ins_b.insert(item, c[item.as_str()].to_string().parse::<i8>().unwrap());
            }
